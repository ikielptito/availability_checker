...
- The Tropicana Valley books (the development as a whole: ledger, costs still to pay, buyers, loans, bank balances, and the loan headline) are NOT in the cockpit. They live on their own site, the Tropicana Valley Books app (a separate Vercel project for Ikiel and Oli), which reads `GET /api/statements?action=finance` here with its own FINANCE_SECRET for the rent and expenses of the four unsold B units (Hostex calendar + Era's statements) and writes the books through the CRM's finance_* actions.
