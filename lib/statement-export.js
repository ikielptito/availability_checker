...
function incomeSheetRows(months) {
  ...
  const head = [...(multi ? ['Month'] : []), 'Unit', 'Guest', 'Stay dates', 'Platform', 'Nights', 'Gross (IDR)', 'Commission (IDR)', 'Net to owner (IDR)'];
  const rows = [head];
  let g = 0, c = 0, n = 0;
...
export function buildStatementWorkbook(pub) {
  ...
  const summary = {
    ...
    rows: [
      ...
      ['Platform / booking commission', -N(t.commission)],
...
// Whole group, optionally one year — from statement_export_data.
export function buildGroupWorkbook({ group, statements }, year) {
  const months = statements.map(st => ({ label: st.period_label || st.period, lines: st.lines || [], payments: st.payments || [] }));
  const sumRows = [['Month', 'Gross (IDR)', 'Commission (IDR)', 'Net income (IDR)', 'Expenses (IDR)', 'Adjustments (IDR)', 'Payout (IDR)', 'Paid (IDR)', 'Balance (IDR)', 'Status'].map(v => ({ v, bold: true }))];
  ...
  const summary = {
    ...
    rows: [
      [{ v: `Samba Realty — Owner Financials — ${group.name}${year ? ` — ${year}` : ''}`, bold: true }],
