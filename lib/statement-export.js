...
function accountRows(account) {
  if (!account || (!account.bank && !account.account_name && !account.account_number && !account.iban)) return [];
  const LABELS = [
    ['country', 'Country'], ['bank', 'Bank'], ['account_name', 'Account name'],
    ['account_number', 'Account number'], ['routing_number', 'ACH routing number'],
    ['sort_code', 'Sort code'], ['bsb', 'BSB'], ['iban', 'IBAN'],
    ['swift', 'SWIFT / BIC'], ['account_type', 'Account type'], ['note', 'Note'],
  ];
  return [
    [''],
    [{ v: 'Payout account', bold: true }],
    ...LABELS.filter(([k]) => account[k]).map(([k, label]) => [label, String(account[k])]),
  ];
}
