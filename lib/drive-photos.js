...
//
// Auth (Vercel env, same values as the kaya-agent-crm project) — two modes:
//
// PREFERRED — OAuth as the Drive owner (works on personal Gmail):
//   GOOGLE_OAUTH_CLIENT_ID / GOOGLE_OAUTH_CLIENT_SECRET / GOOGLE_OAUTH_REFRESH_TOKEN
//   Minted once with dev/get-drive-token.mjs (drive.file scope). Uploads are
//   owned by the user and use THEIR storage quota.
//
...
//   Google gives service accounts ZERO storage quota — folder creation
//   succeeds (folders are weightless) but every file upload fails with
//   "Service Accounts do not have storage quota". That live failure is why
//   the OAuth mode above exists.
//
//   DRIVE_PARENT_FOLDER_ID  parent for all villa folders. In OAuth mode this
//   MUST be the folder the token script created — the drive.file scope only
...
// Upload raw image bytes into the folder as `name`.
export async function uploadBytes({ name, mime, bytes, folderId }) {
  const token = await getAccessToken();
...
export async function uploadPhotoFromUrl({ url, folderId, index }) {
  ...
  return uploadBytes({ name, mime, bytes, folderId });
}
