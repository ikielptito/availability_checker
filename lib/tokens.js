...
export function verifyPortfolioToken(token) {
  // A '+' in a query string decodes to a space (?token=a+b → "a b"); the
  // path form (/r/a+b~sig) keeps it. Accept either separator.
  const t = String(token || '');
  const i = t.lastIndexOf('~');
  if (i < 0 || !/[+ ]/.test(t.slice(0, i))) return null;
...
export function verifyRecordToken(token) {
  const m = String(token || '').match(/^(handover|inspection|visit)\.(\d{1,10})(?:~(owner))?~([0-9a-f]{16})$/);
  if (!m) return null;
