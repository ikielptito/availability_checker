...
// exactly one record.
export function recordSig(type, id) {
  return hmac16(`rec:${type}:${id}`);
}
export function recordToken(type, id) {
  return `${type}.${id}~${recordSig(type, id)}`;
}
export function verifyRecordToken(token) {
  const m = String(token || '').match(/^(handover|inspection)\.(\d{1,10})~([0-9a-f]{16})$/);
  if (!m) return null;
  const [, type, id, sig] = m;
  if (!safeEq(sig, recordSig(type, id))) return null;
