...
// These double as the gallery filter tags (Airbnb-style photo tour). The
// frontend maps them to display labels in public/listing.html (CATEGORY_TAGS).
const CATEGORIES = ['hero-exterior', 'pool', 'living', 'bedroom', 'bathroom', 'kitchen',
  'workspace', 'view', 'outdoor', 'amenity', 'detail', 'utility-junk'];
...
const SCORE_SYSTEM = `You are ranking photos for a Bali villa/apartment rental listing gallery. For each numbered photo, classify it into exactly one category and give an appeal score from 0 to 10 (10 = magazine-quality, makes someone want to book immediately; 0 = should never be shown to a prospective guest).

Categories:
- hero-exterior: the building or villa seen from outside, facades, entrances, architecture.
- pool: swimming pool and its immediate deck/loungers.
- living: living rooms, lounges, sofas, TV areas, dining areas.
- bedroom: bedrooms and beds.
- bathroom: bathrooms, showers, bathtubs, vanities.
- kitchen: kitchens, cooking areas, counters, appliances.
- workspace: a desk, office nook, or dedicated work-from-home setup (desk + chair, monitor). Use this even if the desk sits inside a bedroom or living room, when the photo is clearly focused on the workspace.
- view: scenery, rice fields, ocean, sunset, or the outlook from the property.
- outdoor: gardens, terraces, balconies, outdoor lounges and dining not centred on the pool.
- amenity: gyms, co-working, shared facilities, parking as a feature.
- detail: close-ups of decor, materials, fixtures, styling details.
- utility-junk: staircases, corridors, fire extinguishers, electrical panels or meters, water heaters, storage/closet interiors, back-of-house, signage, construction, or anything a guest would find confusing or off-putting early in a gallery.

Score appeal on lighting, composition, and how inviting the space looks. Flag flaws honestly. Return one entry per photo with n matching the photo's label.`;
...
const BODY_ORDER = ['living', 'kitchen', 'bedroom', 'bathroom', 'workspace', 'pool',
  'outdoor', 'hero-exterior', 'view', 'amenity', 'detail'];
