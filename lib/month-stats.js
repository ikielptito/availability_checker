...
export async function revenueByMonth(units, { from, to }) {
  ...
  for (let t = rangeStart; t < rangeEnd; t = Date.UTC(new Date(t).getUTCFullYear(), new Date(t).getUTCMonth() + 1, 1)) {
    months[monthKey(t)] = { revenue: 0, direct: 0, nights: 0, reservations: 0 };
  }
  // Direct stays Era enters on the calendar herself; a profit view that also
  // reads her direct-rent receipts must not count these twice.
  const isDirect = x => /direct|custom|hostex|manual/i.test(String(x.channel_type || ''));
  ...
  for (const u of units) {
    ...
    for (const x of resv) {
      ...
      for (let t = Math.max(ci, rangeStart); t < Math.min(co, rangeEnd); t += MS_DAY) {
        ...
        if (isDirect(x)) months[k].direct += perNight;
        months[k].nights += 1;
  ...
  for (const k of Object.keys(months)) { months[k].revenue = Math.round(months[k].revenue); months[k].direct = Math.round(months[k].direct); }
  return out;
}
