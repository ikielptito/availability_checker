...
// The enquiry contact is what an agent taps "Visit" to reach, so an intake has
// to carry it — and carry a NAME with it. Casa Suhana went live as a bare
// number because Maya never collected one (16 Aug 2026).
const custom = () => JSON.parse(store.get('custom_properties') || '{}');
check('intake carries the enquiry number',
  custom()['casa-suhana'].waNumber === '447378973820', JSON.stringify(custom()['casa-suhana'].waNumber));

r = await intake({ waNumber: '447378973820', waContactName: 'Suhana Owner',
  data: { name: 'Casa Suhana', area: 'Cemagi', monthly: '41jt' } });
check('intake stores the contact name alongside the number',
  custom()['casa-suhana'].waContactName === 'Suhana Owner' && custom()['casa-suhana'].monthly === '41jt',
  JSON.stringify({ n: custom()['casa-suhana'].waContactName, m: custom()['casa-suhana'].monthly }));

// An update that omits waNumber must not wipe the contact off the listing.
r = await intake({ slug: 'casa-suhana', waNumber: '447378973820', data: { name: 'Casa Suhana', monthly: '42jt' } });
check('an update never wipes the enquiry contact',
  custom()['casa-suhana'].waNumber === '447378973820' && custom()['casa-suhana'].monthly === '42jt',
  JSON.stringify({ wa: custom()['casa-suhana'].waNumber, m: custom()['casa-suhana'].monthly }));
